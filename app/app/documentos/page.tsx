...
export default function HistoricoDocumentos() {
  ...
  const { documentos, carregando, erro, recarregar } = useDocumentos({ busca: buscaAplicada, status: status || undefined, limite: 100 });

  useEffect(() => {
    const espera = window.setTimeout(() => setBuscaAplicada(busca.trim()), 300);
  ...
  return (
    <main className="pagina-app">
      <header className="cabecalho-pagina"><div><p className="rotulo">Arquivo privado</p><h1>Seus documentos</h1><p>Encontre todos os registros ligados a uma pessoa ou a qualquer dado extraído.</p></div></header>
      <div className="barra-filtros">
        <label className="busca-documentos"><Search size={18} /><input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Pessoa, CPF, CNPJ, endereço ou arquivo…" aria-label="Pesquisar nos documentos e dados extraídos" />{busca && <button onClick={() => setBusca("")} aria-label="Limpar busca"><X size={15} /></button>}</label>
      ...
      <p className="retorno-busca" aria-live="polite">
        {buscaAplicada
          ? carregando
            ? "Procurando em todos os dados extraídos…"
            : `${documentos.length} ${documentos.length === 1 ? "documento encontrado" : "documentos encontrados"} para “${buscaAplicada}”`
          : "A pesquisa considera nomes, documentos pessoais, empresas, endereços, datas, valores e o nome do arquivo."}
      </p>

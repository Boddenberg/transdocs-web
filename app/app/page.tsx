...
import { ArrowRight, FileCheck2, Files, Loader2 } from "lucide-react";
...
export default function Bancada() {
  ...
  const { documentos, carregando, erro, recarregar } = useDocumentos({ limite: 10, atualizarEnquantoProcessa: true });
  const nome = String(usuario?.user_metadata?.nome || "").split(" ")[0];
  const revisados = documentos.filter((d) => d.revisado).length;
  const emFila = documentos.filter((d) => d.status === "pendente" || d.status === "processando").length;
  ...
  return (
    <main className="pagina-app pagina-bancada">
      ...
      <ZonaUpload aoEnviar={recarregar} />
      <section className="resumo-bancada">
        ...
        <article><span><Loader2 size={18} className={emFila ? "girando" : ""} /></span><div><strong>{emFila}</strong><small>na fila agora</small></div></article>
      ...
      <section className="secao-app">
        <header className="secao-app__cabecalho"><div><p className="rotulo">Continuidade</p><h2>{emFila ? "Fila e documentos recentes" : "Documentos recentes"}</h2></div><Link href="/app/documentos">Ver histórico <ArrowRight size={15} /></Link></header>

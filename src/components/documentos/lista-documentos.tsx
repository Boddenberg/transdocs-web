      ...
      {documentos.map((documento) => (
        <article className="linha-documento" key={documento.id}>
          <Link className="linha-documento__principal" href={`/app/documentos/${documento.id}`}>
            ...
            <span className="linha-documento__nome"><strong>{documento.nome_original}</strong><small>{formatarTamanho(documento.tamanho_bytes)} · {formatarData(documento.criado_em)}</small></span>
